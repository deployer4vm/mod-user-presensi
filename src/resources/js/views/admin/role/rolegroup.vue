    ...
    <div>
                            ...
                            >
                                ...
                                <span>
                                    {{ Trans.get("lang.add_attribute", { 
                                            attribute: Trans.get("role.group.data_group.name")
                                        })
                                    }}
                                </span>
                            </b-btn>
        ...
        >
            <div slot="modal-title">
                {{ Trans.get('role.group.form_group.name') }} / 
            ...
            <!-- code -->
            <b-form-group
                :label="Trans.get('role.group.data_group.field_name.code')"
            ...
            >
                <b-input
                    ...
                    :placeholder="Trans.get('role.group.form_group.input_description.code')"
                ...
                <invalid-tooltip
                    ...
                    :fieldName="Trans.get('role.group.data_group.field_name.code')"
                />
            </b-form-group>

            <!-- name -->
            <b-form-group
                :label="Trans.get('role.group.data_group.field_name.name')"
                label-align-md="right"
                label-class="pr-md-3"
                :label-cols-md="3"
            >
                <b-input
                    ...
                    :placeholder="Trans.get('role.group.form_group.input_description.name')"
                ...
                <invalid-tooltip
                    ...
                    :fieldName="Trans.get('role.group.data_group.field_name.name')"
                />
            </b-form-group>

            <!-- has model -->
            <b-form-group
                :label="Trans.get('role.group.data_group.field_name.has_model')"
                label-align-md="right"
                label-class="pr-md-3"
                :label-cols-md="3"
            >
                <b-radio-group
                    v-model.trim="data.formData.form.has_model"
                    :options="[
                        {value: 0, text: Trans.get('lang.no')},
            ...
            <!-- model -->
            <b-form-group
                v-if="data.formData.form.has_model"
                :label="Trans.get('role.group.data_group.field_name.model')"
                label-align-md="right"
                label-class="pr-md-3"
                :label-cols-md="3"
            >
                <b-input
                    v-model.trim="data.formData.form.model"
                    :placeholder="Trans.get('role.group.form_group.input_description.model')"
                    :state="$v.data.formData.form.model.$error ? false : null"
                    @change="$v.data.formData.form.model.$touch()"
                    :disabled="isDisabled"
                />

                <invalid-tooltip
                    :inputItem="$v.data.formData.form.model"
                    :fieldName="Trans.get('role.group.data_group.field_name.model')"
                />
            </b-form-group>
            ...
            <!-- can selected on create -->
            <b-form-group
                :label="Trans.get('role.group.data_group.field_name.can_selected_on_create')"
                label-align-md="right"
                label-class="pr-md-3"
                :label-cols-md="3"
            >
                <b-radio-group
                    v-model.trim="data.formData.form.can_selected_on_create"
                    :options="[
                        {value: 0, text: Trans.get('lang.no')},
                        {value: 1, text: Trans.get('lang.yes')}
                    ]"
            ...
            <!-- keterangan -->
            <b-form-group
                :label="Trans.get('role.group.data_group.field_name.description')"
                label-align-md="right"
                label-class="pr-md-3"
                :label-cols-md="3"
                :content-cols-lg="9"
            >
                <b-textarea
					...
					:placeholder="Trans.get('role.group.form_group.input_description.description')"
					:rows="3"
					:max-rows="6"
                    :disabled="isDisabled"
				/>
            ...
            <b-form-group
                ...
                :label="Trans.get('role.group.data_group.field_name.locked_data_mode')"
                label-align-md="right"
                label-class="pr-md-3"
                :label-cols-md="3"
            >
...
<script>
import { required, requiredIf } from "node_modules/vuelidate/lib/validators";
...
export default {
                ...
                form: {
                    ...
                    model: {
                        required: requiredIf(function () {
                            return this.data.formData.form.has_model;
                        }),
                    }
                }
            }
    ...
    computed: {
        data: {
            get() {
                return this.$store.state.moduserView.dataRoleGroup;
            },
            set(value) {
                this.$store.commit("moduserView/setDataRoleGroup", value);
            }
        },
        listData() {
            return this.Repo('moduserRoleGroup').listData;
        },  
        oneData() {
            return this.Repo('moduserRoleGroup').oneData;
        },
        pageTitle() {
            return this.Trans.get("role.group.name");
        },
        totalPages() {
        ...
        fields() {
            return [
                ...
                {
                    key: "code",
                    label: this.Trans.get('role.group.data_group.field_name.code'),
                    thClass: "text-left",
                    tdClass: "text-left"
                },
                {
                    key: "name",
                    label: this.Trans.get('role.group.data_group.field_name.name'),
                    thClass: "text-left",
                    tdClass: "text-left"
                },
                {
                    key: "description",
                    label: this.Trans.get('role.group.data_group.field_name.description'),
                    thClass: "text-left",
                    tdClass: "text-left"
                },
                {
    ...
    methods: {
                    ...
                    this.Web.showAlert({
                        ...
                        text:
                            this.Trans.get("alert.read_failed", {
                                attribute: this.Trans.get("role.group.data_group.name"),
                            }) +
                            "<br>\n" +
                            res.message,
                        type: "warning",
        ...
        updateData(data) {
            ...
            this.Web.setLoadingPage(true);
            this.Repo('moduserRoleGroup').update({
                    id: data.id,
                    data: data,
                })
                .then((res) => {
        ...
        deleteData(id) {
            ...
            this.Web.showAlert({
                ...
                onOk: () => {
                    this.Repo('moduserRoleGroup').delete({ id: id })
                        .then((res) => {
                            this.Web.showAlert({
                                title: this.Trans.get("alert.success_title"),
