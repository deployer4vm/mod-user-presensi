  ...
  <div>
    ...
    <b-container fluid>
          ...
          <div class="d-flex flex-column flex-md-row justify-content-between">
            <div class="d-flex flex-wrap flex-md-nowrap align-items-center">
              <b-btn
                ...
                variant="default btn-sm btn-collapse"
              >
                ...
                <span>
                  {{
                    Trans.get("lang.add_attribute", {
                      attribute: Trans.get("user.user_group.data_group.name"),
                    })
                  }}
                </span>
              </b-btn>
      ...
      <b-card class="my-3" no-body>
        ...
        <b-card-header>
          ...
          >
            <h5 class="my-1">
              {{ Trans.get("user.user_group.table_group.name") }}
            </h5>
          </div>
        </b-card-header>
        <!-- / Header -->
          ...
          >
            ...
            <template v-slot:cell(no)="row">
              {{
                row.index +
                1 +
                (data.listData.curPage - 1) * data.listData.perPage
              }}
            </template>
            ...
            <template v-slot:cell(actions)="row">
              <div class="d-flex align-items-center justify-content-center">
                <b-btn
                  v-if="
                    (UserAuth.hasAccess(data.accessRuleKey, 'u') &&
                      row.item.locked_data_mode != 2) ||
                    UserAuth.isWebdev()
                  "
                ...
                <b-btn
                  v-if="
                    UserAuth.hasAccess(data.accessRuleKey, 'd') &&
                    row.item.locked_data_mode == 0
                  "
    ...
    >
      <div slot="modal-title">
        {{ Trans.get("user.user_group.form_group.name") }} /
      ...
      <!-- code -->
      <b-form-group
        :label="Trans.get('user.user_group.data_group.field_name.code')"
      ...
      >
        <b-input
          ...
          :placeholder="
            Trans.get('user.user_group.form_group.input_description.code')
          "
        ...
        <invalid-tooltip
          ...
          :fieldName="Trans.get('user.user_group.data_group.field_name.code')"
        />
      </b-form-group>

      <!-- name -->
      <b-form-group
        :label="Trans.get('user.user_group.data_group.field_name.name')"
        label-align-md="right"
        label-class="pr-md-3"
        :label-cols-md="3"
      >
        <b-input
          ...
          :placeholder="
            Trans.get('user.user_group.form_group.input_description.name')
          "
        ...
        <invalid-tooltip
          ...
          :fieldName="Trans.get('user.user_group.data_group.field_name.name')"
        />
      </b-form-group>

      <!-- keterangan -->
      <b-form-group
        :label="Trans.get('user.user_group.data_group.field_name.description')"
        label-align-md="right"
        label-class="pr-md-3"
        :label-cols-md="3"
        :content-cols-lg="9"
      >
        <b-textarea
          ...
          :placeholder="
            Trans.get(
              'user.user_group.form_group.input_description.description'
            )
          "
          :rows="3"
          :max-rows="6"
      ...
      <b-form-group
        ...
        :label="
          Trans.get('user.user_group.data_group.field_name.locked_data_mode')
        "
        label-align-md="right"
        label-class="pr-md-3"
        :label-cols-md="3"
...
export default {
  ...
  computed: {
    data: {
      get() {
        return this.$store.state.moduserView.dataUserGroup;
      },
      set(value) {
        this.$store.commit("moduserView/setDataUserGroup", value);
      },
    },
    listData() {
      return this.Repo("moduserUserGroup").listData;
    },
    oneData() {
      return this.Repo("moduserUserGroup").oneData;
    },
    pageTitle() {
      return this.Trans.get("user.user_group.name");
    },
    totalPages() {
    ...
    fields() {
      return [
        ...
        {
          key: "code",
          label: this.Trans.get("user.user_group.data_group.field_name.code"),
          thClass: "text-left",
          tdClass: "text-left",
        },
        {
          key: "name",
          label: this.Trans.get("user.user_group.data_group.field_name.name"),
          thClass: "text-left",
          tdClass: "text-left",
        },
        {
          key: "description",
          label: this.Trans.get(
            "user.user_group.data_group.field_name.description"
          ),
          thClass: "text-left",
          tdClass: "text-left",
        },
    ...
    canEditLockedDataMode() {
      return (
        this.UserAuth.hasAccess(
          this.data.accessRuleKey + ".can_edit_locked_data_mode"
        ) || this.UserAuth.isWebdev()
      );
    },
    isDisabled() {
      return (
        !this.data.formData.isAdd &&
        this.data.formData.form.locked_data_mode == 2
      );
    },
  },
  ...
  methods: {
    ...
    loadList(curPage = 1, q = "", orderBy = false, sortDesc = false) {
      ...
      if (orderBy != false) {
        ...
        this.data.listData.loadParams.orderType = this.data.listData.sortDesc
          ? "DESC"
          : "ASC";
      }
          ...
          this.Web.showAlert({
            ...
            text:
              this.Trans.get("alert.read_failed", {
                attribute: this.Trans.get("user.user_group.data_group.name"),
              }) +
              "<br>\n" +
              res.message,
            type: "warning",
    ...
    showForm(isAdd = true, id = 0) {
      ...
      if (!isAdd) {
        ...
        this.data.formData.form = JSON.parse(
          JSON.stringify(this.data.formData.formEmpty)
        );
        this.$bvModal.show("modals-form");
      }
    },
              ...
              this.Web.showAlert({
                ...
                text:
                  this.Trans.get("alert.delete_failed", {
                    attribute: this.Trans.get(
                      "user.user_group.data_group.name"
                    ),
                  }) +
                  "<br>\n" +
                  res.message,
  ...
  created() {
    ...
    this.selectLockedDataMode = [
      {
        text: this.Trans.get(
          "user.user_group.data_group.label.locked_data_mode_0"
        ),
        value: 0,
      },
      {
        text: this.Trans.get(
          "user.user_group.data_group.label.locked_data_mode_1"
        ),
        value: 1,
      },
      {
        text: this.Trans.get(
          "user.user_group.data_group.label.locked_data_mode_2"
        ),
        value: 2,
      },
    ];
