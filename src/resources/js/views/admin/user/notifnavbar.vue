<template>
  <b-nav-item-dropdown
    no-caret
    :right="!isRTL"
    class="demo-navbar-notifications mr-lg-2"
  >
    <template slot="button-content">
      ...
      <span class="d-lg-none align-middle"
        >&nbsp; {{ Trans.get("notif.notification_title") }}</span
      >
    </template>
    ...
    <b-list-group flush>
      ...
      >
        ...
        <div class="media-body line-height-condenced ml-3">
          <div
            :class="
              item.read_at == null ? 'text-dark font-weight-bold' : 'text-muted'
            "
          >
            {{ item.data.subject }}
          </div>
...
export default {
  ...
  created() {
    if (this.UserAuth.isLogin()) {
      ...
      if (this.isUsingPusher && this.isNotifPusher) {
        Echo.channel(
          this.AppConfig.packageLocal.moduser.notification.services.pusher
            .config.user_channel_prefix + this.UserAuth.getUser("id")
        ).listen(
          "." +
            this.AppConfig.packageLocal.moduser.notification.services.pusher
              .config.user_event,
  ...
  computed: {
    ...
    isNotifPusher() {
      return this.AppConfig.packageLocal.moduser.notification.services.pusher
        .enable
        ? true
        : false;
    },
      ...
      this.LocalApi.get(
        ...
        .then((res) => {
          if (res) {
            ...
            var subject = "";
            var newNotifCount =
              that.notif.summary.unread_count - that.lastNotifCount;
