<template>
  <div>
    <div class="media align-items-center py-3 mb-3">
      <img
        :src="`${publicUrl}img/avatars/${userData.avatar}`"
        alt=""
      ...
      <div class="media-body ml-4">
        ...
        <b-btn variant="default icon-btn" size="sm"
        ...
        <table class="table user-view-table m-0">
          <tbody>
            ...
            <tr>
              <td>Status:</td>
              <td>
                <b-badge variant="outline-success" v-if="userData.status === 1"
                  >Active</b-badge
                >
                <b-badge variant="outline-danger" v-if="userData.status === 2"
                  >Banned</b-badge
                >
                <b-badge variant="outline-default" v-if="userData.status === 3"
                  >Deleted</b-badge
                >
              </td>
            </tr>
        ...
        <b-table :items="userData.permissions" class="card-table m-0">
          <template slot="read" slot-scope="data">
            <span
              class="ion"
              :class="{
                'ion-md-checkmark text-primary': data.item.read,
                'ion-md-close text-light': !data.item.read,
              }"
            ></span>
          </template>
          <template slot="write" slot-scope="data">
            <span
              class="ion"
              :class="{
                'ion-md-checkmark text-primary': data.item.write,
                'ion-md-close text-light': !data.item.write,
              }"
            ></span>
          </template>
          <template slot="create" slot-scope="data">
            <span
              class="ion"
              :class="{
                'ion-md-checkmark text-primary': data.item.create,
                'ion-md-close text-light': !data.item.create,
              }"
            ></span>
          </template>
          <template slot="delete" slot-scope="data">
            <span
              class="ion"
              :class="{
                'ion-md-checkmark text-primary': data.item.delete,
                'ion-md-close text-light': !data.item.delete,
              }"
            ></span>
          </template>
        </b-table>
        ...
        <table class="table user-view-table m-0">
          <tbody>
            <tr v-if="userData.info.twitter">
              <td>Twitter:</td>
              <td>
                <a href="javascript:void(0)">{{ userData.info.twitter }}</a>
              </td>
            </tr>
            <tr v-if="userData.info.facebook">
              <td>Facebook:</td>
              <td>
                <a href="javascript:void(0)">{{ userData.info.facebook }}</a>
              </td>
            </tr>
            <tr v-if="userData.info.google">
              <td>Google+:</td>
              <td>
                <a href="javascript:void(0)">{{ userData.info.google }}</a>
              </td>
            </tr>
            <tr v-if="userData.info.linkedin">
              <td>LinkedIn:</td>
              <td>
                <a href="javascript:void(0)">{{ userData.info.linkedin }}</a>
              </td>
            </tr>
            <tr v-if="userData.info.instagram">
              <td>Instagram:</td>
              <td>
                <a href="javascript:void(0)">{{ userData.info.instagram }}</a>
              </td>
            </tr>
          </tbody>
        </table>
...
<script>
import * as numeral from "node_modules/numeral";
