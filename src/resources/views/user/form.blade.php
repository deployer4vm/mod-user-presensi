...
@section('content')
    ...
    $isSetSatker = false;
    if (isset($data['id'])) {
        $isSetSatker = \Facades\App\MainApp\Modules\moduser\Repositories\UserRepo::isHasRole($data['id'], 'pejabat_approval') || \Facades\App\MainApp\Modules\moduser\Repositories\UserRepo::isHasRole($data['id'], 'admin_satker') || \Facades\App\MainApp\Modules\moduser\Repositories\UserRepo::isHasRole($data['id'], 'pimpinan2') || \Facades\App\MainApp\Modules\moduser\Repositories\UserRepo::isHasRole($data['id'], 'pimpinan3');
    ...
    <div id="vueFormPegawai">
        <h4 class="d-flex justify-content-between align-items-center w-100 font-weight-bold py-1 mb-4">
            <div><span class="text-muted font-weight-light">Setup / <a href="{{ route('user.list') }}">Managemen User</a>
                    /</span> {{ $viewMode ? 'View User ' . (isset($data['name']) ? $data['name'] : '') : 'User Form' }}</div>
        </h4>
        @include('alert')
        <div class="card">
            <div class="card-body">
                <div class="d-flex flex-row">
                    ...
                    <div style="width: 100%;">
                        <form action="{{ $mode == 'add' ? route('user.create') : route('user.update', ['id' => $data['id']]) }}"
                            method="post">
                            @if ($mode == 'edit')
                                        ...
                                        @if ($role['role_code'] == 'pegawai_ptt')
                                            @if ($isPegawai || $viewMode)
                                                <label class="custom-control custom-checkbox">
                                                    <input name="role_code[]" type="checkbox" class="custom-control-input"
                                                        value="{{ $role['role_code'] }}" checked readonly="true"
                                                        disabled="true">
                                                    <span
                                                        class="custom-control-label">{{ '[' . $role['role_code'] . '] ' . $role['name'] }}</span>
                                                </label>
                                        ...
                                        @else
                                            <label class="custom-control custom-checkbox">
                                                <input name="role_code[]" type="checkbox" class="custom-control-input"
                                                    ...
                                                    {{ isset($data['role']) && strpos($data['role'], ';' . $role['role_code'] . ';') !== false ? 'checked' : '' }}
                                                    {{ $viewMode ? 'disabled' : '' }}>
                                                <span
                                                    class="custom-control-label">{{ '[' . $role['role_code'] . '] ' . $role['name'] }}</span>
                                            </label>
                            ...
                            @if ($isSetSatker)
                                <div class="form-group row">
                                    ...
                                    @include('instansi.selectnested',['data'=>$instansi['data'],'induk'=>0,'maxEselon'=>4])
                                        ...
                                        <input type="hidden" id="form-instansi-id" name="profile[instansi_id]"
                                            value="{{ isset($data['profile']['instansi_id']) ? $data['profile']['instansi_id'] : 0 }}">
                                        <template v-for="(instansiItem, key) in instansi">
                                            <select @change="changeInstansi(key)" class="form-control mb-2"
                                                v-model="instansiIds[key+1]" {{ $isPegawai || $viewMode ? 'disabled' : '' }}>
